...
import { currentDate, eclipseAddress, season2LaunchDate, signUpSteps } from "@/lib/helpers";
    ...
    <div className="bg-oslo-gray/[.22] rounded-[20px] flex flex-row md:flex-col md:gap-4 justify-between items-center md:text-center w-[849px] md:w-screen md:max-w-9/10 md:m-auto h-[113px] md:h-auto px-10 md:px-4 md:py-6">
      ...
      {(signupStepCompleted[signUpSteps.WALLET] && address) &&
        <div className="flex justify-center items-center gap-2 border border-smokey-gray rounded-full w-[163px] py-2.5">
          ...
          {currentDate >= season2LaunchDate && !signupStepCompleted[signUpSteps.WALLET + 1] && (isCreating || isRetrieving) &&
            <Spinner />
          }
        </div>
      }
